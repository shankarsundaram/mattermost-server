...
func (_m *TeamStore) GetNewTeamMembersSince(teamID string, since int64, offset int, limit int) ([]*model.NewTeamMember, int64, error) {
	ret := _m.Called(teamID, since, offset, limit)

	var r0 []*model.NewTeamMember
	if rf, ok := ret.Get(0).(func(string, int64, int, int) []*model.NewTeamMember); ok {
		r0 = rf(teamID, since, offset, limit)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*model.NewTeamMember)
		}
	}
